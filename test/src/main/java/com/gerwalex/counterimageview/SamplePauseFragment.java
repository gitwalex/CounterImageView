...
public class SamplePauseFragment extends SampleFragment {
    ...
    protected void createTracks() {
        ...
        SeriesItem seriesBack1Item = new SeriesItem.Builder(Color.parseColor("#11000000")).setRange(0, seriesMax, 0)
        ...
        SeriesItem series1Item = new SeriesItem.Builder(Color.parseColor("#FFFFC107")).setRange(0, seriesMax, 0)
                .setLineWidth(getDimension(36)).setInterpolator(new LinearInterpolator()).build();
        series1Item.addArcSeriesItemListener(new SeriesItem.SeriesItemListener() {
            @Override
            public void onSeriesItemAnimationProgress(float percentComplete, float currentPosition) {
                decoView.getChartSeries(mPieIndex)
                        .setPosition(percentComplete < 1.0f ? percentComplete * seriesMax : 0f);
            }
        ...
        final TextView textPause = view.findViewById(R.id.textViewPause);
        ...
        Button buttonEmpty = view.findViewById(R.id.buttonEmpty);
        buttonEmpty.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                decoView.addEvent(new DecoEvent.Builder(0).setIndex(mSeries1Index).setListener(eventListener).build());
            }
        });
        Button buttonFifty = view.findViewById(R.id.buttonFifty);
        buttonFifty.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                decoView.addEvent(
                        new DecoEvent.Builder(seriesMax * 0.5f).setIndex(mSeries1Index).setListener(eventListener)
                                .build());
            }
        });
        Button buttonFull = view.findViewById(R.id.buttonFull);
        buttonFull.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                decoView.addEvent(
                        new DecoEvent.Builder(seriesMax).setIndex(mSeries1Index).setListener(eventListener).build());
            }
        });
