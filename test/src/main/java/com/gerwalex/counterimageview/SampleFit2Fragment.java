...
public class SampleFit2Fragment extends SampleFragment {
    ...
    private void addAnimation(final DecoView arcView, int series, float moveTo, int delay, final ImageView imageView,
                              final int imageId, final String format, final int color, final boolean restart) {
            ...
            public void onSeriesItemAnimationProgress(float percentComplete, float currentPosition) {
                if (mProgress != null) {
                    if (mProgress.contains("%%")) {
                        view.setText(
                                String.format(mProgress, (1.0f - (currentPosition / seriesItem.getMaxValue())) * 100f));
                    } else {
    ...
    protected void createTracks() {
        ...
        SeriesItem seriesBack1Item =
                new SeriesItem.Builder(COLOR_BACK).setRange(0, mSeriesMax, mSeriesMax).setLineWidth(getDimension(46))
                        .build();
        decoView.addSeries(seriesBack1Item);
        SeriesItem series1Item =
                new SeriesItem.Builder(COLOR_NEUTRAL).setRange(0, mSeriesMax, 0).setInitialVisibility(false)
                        .setLineWidth(getDimension(46)).setCapRounded(true)
                        .addEdgeDetail(new EdgeDetail(EdgeDetail.EdgeType.EDGE_INNER, COLOR_EDGE, 0.3f))
                        .setShowPointWhenEmpty(true).build();
        mSeries1Index = decoView.addSeries(series1Item);
        TextView textListener = view.findViewById(R.id.textProgress);
    ...
    protected void setupEvents() {
        ...
        addAnimation(arcView, mSeries1Index, 79.7f, 2000, imgView, R.drawable.ic_fit_bike, "Cycle %.0f Km", COLOR_GREEN,
                false);
        addAnimation(arcView, mSeries1Index, 16.4f, 9000, imgView, R.drawable.ic_fit_run, "Run %.1f Km", COLOR_YELLOW,
                false);
        addAnimation(arcView, mSeries1Index, 58f, 16000, imgView, R.drawable.ic_fit_gym, "Gym %.0f min", COLOR_PINK,
                false);
        addAnimation(arcView, mSeries1Index, 3.38f, 23000, imgView, R.drawable.ic_fit_swim, "Swim %.2f Km", COLOR_BLUE,
                true);
    }
