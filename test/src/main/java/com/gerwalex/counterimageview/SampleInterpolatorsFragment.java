...
public class SampleInterpolatorsFragment extends SampleFragment {
    ...
    private void createTracks(int arcViewId, Interpolator interpolator, int color) {
        ...
        final DecoView decoView = view.findViewById(arcViewId);
        if (decoView == null) {
            return;
        }
        decoView.deleteAll();
        ...
        SeriesItem arcBackTrack =
                new SeriesItem.Builder(Color.argb(255, 228, 228, 228)).setRange(0, mSeriesMax, mSeriesMax)
                        .setLineWidth(getDimension(12f)).build();
        decoView.addSeries(arcBackTrack);
        SeriesItem seriesItem1 = new SeriesItem.Builder(color).setRange(0, mSeriesMax, 0).setInterpolator(interpolator)
                .setLineWidth(getDimension(12f)).setSpinDuration(5000).setSpinClockwise(false).build();
    ...
    private void setupEvents(final int arcId) {
        ...
        final DecoView decoView = view.findViewById(arcId);
