...
import com.gerwalex.counterimageview.charts.ChartSeries;
import com.gerwalex.counterimageview.charts.DecoDrawEffect;
import com.gerwalex.counterimageview.charts.LineArcSeries;
import com.gerwalex.counterimageview.charts.LineSeries;
import com.gerwalex.counterimageview.charts.PieSeries;
import com.gerwalex.counterimageview.charts.SeriesItem;
import com.gerwalex.counterimageview.events.DecoEvent;
import com.gerwalex.counterimageview.events.DecoEventManager;
import com.gerwalex.counterimageview.util.GenericFunctions;
...
public class DecoView extends View implements DecoEventManager.ArcEventManagerListener {
     ...
     * executed immediately or if the event has a DecoEvent#mDelay set then it will be
    ...
    private void createVisualEditorTrack() {
        if (isInEditMode()) {
            addSeries(new SeriesItem.Builder(Color.argb(255, 218, 218, 218)).setRange(0, 100, 100)
                    .setLineWidth(mDefaultLineWidth).build());
            addSeries(new SeriesItem.Builder(Color.argb(255, 255, 64, 64)).setRange(0, 100, 25)
                    .setLineWidth(mDefaultLineWidth).build());
        }
    }
        ...
        if (mChartSeries != null) {
            if (mChartSeries.size() <= event.getIndexPosition()) {
                throw new IllegalArgumentException(
                        "Invalid index: Position out of range (Index: " + event.getIndexPosition() + " Series Count: " +
                                mChartSeries.size() + ")");
            }
            ...
            if (index >= 0 && index < mChartSeries.size()) {
                ...
                Log.e(TAG, "Ignoring move request: Invalid array index. Index: " + index + " Size: " +
                        mChartSeries.size());
            }
        }
    }
    ...
    private void recalcLayout() {
        ...
        mArcBounds = new RectF(offsetLineWidth + paddingLeft, offsetLineWidth + paddingTop,
                mCanvasWidth - offsetLineWidth - paddingRight, mCanvasHeight - offsetLineWidth - paddingBottom);
    ...
    public enum HorizGravity {
        GRAVITY_HORIZONTAL_LEFT, GRAVITY_HORIZONTAL_CENTER, GRAVITY_HORIZONTAL_RIGHT, GRAVITY_HORIZONTAL_FILL
    }
    ...
    public enum VertGravity {
        GRAVITY_VERTICAL_TOP, GRAVITY_VERTICAL_CENTER, GRAVITY_VERTICAL_BOTTOM, GRAVITY_VERTICAL_FILL
    }
}