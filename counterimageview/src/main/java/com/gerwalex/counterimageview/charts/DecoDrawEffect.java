...
import com.gerwalex.counterimageview.DecoView;
...
public class DecoDrawEffect {
     ...
     */
    public void draw(@NonNull Canvas canvas, @NonNull RectF bounds, float percentComplete, float startAngle,
                     float sweepAngle) {
        switch (mEffectType) {
    ...
    private void drawExplodeLine(@NonNull Canvas canvas, RectF bounds, float radiusStart, float radiusEnd,
                                 float angleInDegrees, float percentComplete, boolean compatMode) {
        ...
        if (!compatMode) {
            ...
            float radius = (bounds.width() * EXPLODE_CIRCLE_MIN) +
                    ((bounds.width() * EXPLODE_CIRCLE_MAX - bounds.width() * EXPLODE_CIRCLE_MIN) * percentComplete);
     ...
     */
    public void drawMoveToCenter(@NonNull Canvas canvas, RectF bounds, float percentComplete, float startAngle,
                                 float sweepAngle) {
        // Animation moves outward from center to outside
        final boolean moveOutward =
                mEffectType == EffectType.EFFECT_SPIRAL_OUT || mEffectType == EffectType.EFFECT_SPIRAL_OUT_FILL;
        // Animation spins in a clockwise direction
        final boolean spinClockwise =
                mEffectType != EffectType.EFFECT_SPIRAL_IN && mEffectType != EffectType.EFFECT_SPIRAL_EXPLODE;
        ...
        float rotateAmount =
                (mEffectType == EffectType.EFFECT_SPIRAL_OUT_FILL) ? baseRotateAngle + 360f : baseRotateAngle;
        ...
        if (mEffectType == EffectType.EFFECT_SPIRAL_OUT_FILL) {
            if ((rotateAmount * percentComplete) > (rotateAmount - 360f)) {
                ...
                if (percent > min) {
                    ...
                    mSpinBounds.inset(halfWidth * adjustedPercentage, halfHeight * adjustedPercentage);
                }
            }
        } else {
        ...
        canvas.drawArc(mSpinBounds, newAngle, sweep, false, mPaint);
    }
    ...
    public void drawText(@NonNull Canvas canvas, RectF bounds, float percentComplete) {
        if (mText != null && mText.length() > 0) {
            ...
            if (percentComplete > startFadePercent) {
                int alphaText = (int) (MAX_ALPHA -
                        (MAX_ALPHA * ((percentComplete - startFadePercent) / (1.0f - startFadePercent))));
                mPaintText.setAlpha(alphaText);
            }
            // Calculate a centered position for the text
            final float xPos = bounds.left + (bounds.width() / 2);
            final float yPos =
                    (bounds.top + (bounds.height() / 2)) - ((mPaintText.descent() + mPaintText.ascent()) / 2);
    ...
    public boolean postExecuteVisibility() {
        return (mEffectType == EffectType.EFFECT_SPIRAL_OUT) || (mEffectType == EffectType.EFFECT_SPIRAL_OUT_FILL);
    }
