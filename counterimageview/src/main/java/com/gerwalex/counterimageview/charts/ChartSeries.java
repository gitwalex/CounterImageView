...
abstract public class ChartSeries {
    ...
    protected float calcCurrentPosition(float start, float end, float min, float max, float percent) {
        ...
        if ((mDrawMode == DecoEvent.EventType.EVENT_HIDE) || (mDrawMode == DecoEvent.EventType.EVENT_SHOW) ||
    ...
    public RectF drawLabel(Canvas canvas, RectF bounds, float anglePercent) {
        ...
        if (mSeriesItem.getSeriesLabel() != null) {
            return mSeriesItem.getSeriesLabel()
                    .draw(canvas, bounds, anglePercent, getPositionPercent(), mPositionCurrentEnd);
        }
        return null;
    }
    ...
    public void reset() {
        ...
        mPaint = new Paint();
        mPaint.setColor(mSeriesItem.getColor());
        mPaint.setStyle((mSeriesItem.getChartStyle() == SeriesItem.ChartStyle.STYLE_DONUT) ? Paint.Style.STROKE :
                Paint.Style.FILL);
        mPaint.setStrokeWidth(mSeriesItem.getLineWidth());
    ...
    protected float verifyMinSweepAngle(final float angle) {
        return (Math.abs(angle) < getMinSweepAngle() && getSeriesItem().showPointWhenEmpty()) ? getMinSweepAngle() :
                angle;
    }
}